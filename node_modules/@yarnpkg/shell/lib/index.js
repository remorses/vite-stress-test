...
exports.execute = exports.globUtils = void 0;
...
const BUILTINS = new Map([
    ...
    [`__ysh_run_procedure`, async (args, opts, state) => {
            ...
            const exitCode = await pipe_1.start(procedure, {
                stdin: new pipe_1.ProtectedStream(state.stdin),
                stdout: new pipe_1.ProtectedStream(state.stdout),
                stderr: new pipe_1.ProtectedStream(state.stderr),
            }).run();
            return exitCode;
        }],
            ...
            while (args[t] !== `--`) {
                ...
                for (let u = t; u < last; ++t, ++u) {
                    switch (type) {
                        ...
                        case `>`:
                            {
                                outputs.push(opts.baseFs.createWriteStream(fslib_1.ppath.resolve(state.cwd, fslib_1.npath.toPortablePath(args[u]))));
                            }
                            break;
                        case `>>`:
                            {
                                outputs.push(opts.baseFs.createWriteStream(fslib_1.ppath.resolve(state.cwd, fslib_1.npath.toPortablePath(args[u])), { flags: `a` }));
                            }
                            break;
                        case `>&`:
                            {
    ...
    while (current) {
        ...
        if (pipeType === null) {
            ...
            switch (pipeType) {
                case `|`:
                    {
                        execution = execution.pipeTo(action, pipe_1.Pipe.STDOUT);
                    }
                    break;
                case `|&`:
                    {
                        execution = execution.pipeTo(action, pipe_1.Pipe.STDOUT | pipe_1.Pipe.STDERR);
                    }
                    break;
            }
        }
